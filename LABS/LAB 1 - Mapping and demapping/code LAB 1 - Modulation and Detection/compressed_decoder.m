...
height  = ceil(image_size(1)/8)*8; % here height -> image_size(1)
width   = ceil(image_size(2)/8)*8; % here width -> image_size(2)
...
for rr=1:8:height % go through rows
    for cc=1:8:width % go through columns
        ...
        k = k + 1; % lui il est vicieux de fou malade, pas sympa mais clairement il rendait tout le code faux
    end
end
