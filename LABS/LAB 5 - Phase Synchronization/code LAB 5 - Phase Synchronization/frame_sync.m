...
% Do not simply return the index where T exceeds the threshold for the first time. 
% Since the signal is oversampled, so will be the peak in the correlator output. 
% So once we have detected a peak, we keep on processing the next L samples and 
% return the index where the test statistic takes on the maximum value.
